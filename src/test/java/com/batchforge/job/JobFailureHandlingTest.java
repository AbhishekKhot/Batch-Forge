...
class JobFailureHandlingTest {
    ...
    @Test
    void processingFailureExhaustsRetriesAndMarksJobFailed() throws Exception {
        doThrow(new RuntimeException("boom")).when(importProcessor).process(any());
    ...
    @Test
    void markFailedDoesNotOverrideCompletedJob() {
        OrgUser u = newOrgUser();
        Job done = new Job(u.orgId(), u.userId(), u.orgId() + "/" + UUID.randomUUID() + "/source.csv");
        done.markQueued();
        done.markProcessing();
        done.markCompleted();
        done = jobRepository.save(done);

        processing.markFailed(done.getId());

        assertThat(jobRepository.findById(done.getId()).orElseThrow().getStatus()).isEqualTo(JobStatus.COMPLETED);
    }
